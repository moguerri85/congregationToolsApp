...
def update_week_display(app, tipologia_nome):
    try:
        ...
        # Crea e visualizza i quadrati per ogni giorno della settimana
        ...
        for day in days_of_week:
            ...
            if tipologia_id in app.tipologia_schedule and day in app.tipologia_schedule[tipologia_id]:
                fasce = app.tipologia_schedule[tipologia_id][day]
            ...
            square_button.clicked.connect(lambda _, d=day, t=tipologia_id, b=square_button: on_square_click(app, d, t, b))
...
def add_fascia(app, day, tipologia_id, fascia_list_widget):
    try:
        text, ok = QInputDialog.getText(app, "Aggiungi Fascia Oraria", "Inserisci fascia oraria (es. 09:00 - 10:00):")
        if ok and text:
            if tipologia_id not in app.tipologia_schedule:
                app.tipologia_schedule[tipologia_id] = {}
            if day not in app.tipologia_schedule[tipologia_id]:
                app.tipologia_schedule[tipologia_id][day] = []
            
            app.tipologia_schedule[tipologia_id][day].append(text)
            fascia_list_widget.addItem(text)
            update_day_square(app, day, tipologia_id)
...
def delete_fascia(app, day, tipologia_id, fascia_list_widget):
    try:
        selected_item = fascia_list_widget.currentItem()
        if selected_item:
